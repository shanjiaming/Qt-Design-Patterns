...
cout << str;
QFile data("mydata");
data.open( QIODevice::WriteOnly);//Creates (or overwrites) a disk file for output.
QTextStream out(&data); //An output file stream.
out << str ;
data.close();
cout << "Read data from the file - watch for errors." << endl;
if(data.open( QIODevice::ReadOnly)) {//Make sure the file exists before attempting to read.
